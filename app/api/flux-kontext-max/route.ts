...
export async function POST(req: Request) {
  // Require authentication (gate this route like flux-multi)
  ...
  let body: FluxKontextInput;
  try {
    body = (await req.json()) as FluxKontextInput;
  } catch {
  ...
  const rawUrl = (body?.image_url ?? "").toString().trim();
  if (!rawUrl) return badRequest("Field 'image_url' is required");
  if (!isHttpUrl(rawUrl)) {
    return badRequest("image_url must be an absolute http(s) URL (no blob: or data:)");
  }
  ...
  // --- Credit policy ---
  const policy = await getCreditPolicy(supabase, "flux-kontext-max");
