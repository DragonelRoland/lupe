...
export async function POST(req: Request) {
  ...
  let body: HailouFastI2VInput;
  try {
    body = (await req.json()) as HailouFastI2VInput;
  } catch {
  ...
  const input: HailouFastI2VInput = {
    prompt,
    image_url,
    duration,
    prompt_optimizer,
  ...
  // --- Credit policy ---
  const policy = await getCreditPolicy(supabase, "hailou-fast-i2v");
