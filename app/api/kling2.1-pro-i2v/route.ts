...
export async function POST(req: Request) {
  ...
  let body: KlingProI2VInput;
  try {
    body = (await req.json()) as KlingProI2VInput;
  } catch {
  ...
  const tail_image_url = (body?.tail_image_url ?? "").toString().trim();
  // Validate tail_image_url only if provided
  if (tail_image_url && (!/^https?:\/\//i.test(tail_image_url) && !/^data:/i.test(tail_image_url))) {
    return badRequest("tail_image_url must be http(s) or data URI");
  }
  ...
  // --- Credit policy ---
  const policy = await getCreditPolicy(supabase, "kling2.1-pro-i2v");
