...
export async function POST(req: Request) {
  ...
  let body: KlingMasterI2VInput;
  try {
    body = (await req.json()) as KlingMasterI2VInput;
  } catch {
  ...
  const input: KlingMasterI2VInput = {
    prompt,
    image_url,
    duration,
    negative_prompt,
  ...
  // --- Credit policy ---
  const policy = await getCreditPolicy(supabase, "kling2.1-master-i2v");
  ...
  try {
    ...
    if (shouldEnforce) {
      const { error: deductErr } = await supabase.rpc("rpc_deduct_credits", {
        p_model_id: "kling2.1-master-i2v",
